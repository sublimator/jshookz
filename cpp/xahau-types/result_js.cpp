...
JSValue
// @binding provider:Result.okOrHandle
resultOkOrHandle(
    JSContext* ctx,
    JSValueConst thisValue,
    int argc,
...
JSValue
// @binding provider:Result.okMapOr
resultOkMapOr(
    JSContext* ctx,
    JSValueConst thisValue,
    int argc,
...
JSValue
// @binding provider:VoidResult.moot
resultMoot(
    JSContext* ctx,
    JSValueConst thisValue,
    int,
...
bool
result_finish(JSContext* ctx, JSValueConst result)
{
    return JS_PreventExtensions(ctx, result) >= 0;
}

JSValue
result_error(JSContext* ctx, char const* domain)
{
...
JSValue
result_failure(JSContext* ctx, JSValue error, bool effect)
{
...
{
    qjs::OwnedValue error(ctx, result_error(ctx, "uint"));
    if (error.isException())
        return error.release();
    if (JS_DefinePropertyValueStr(
            ctx,
