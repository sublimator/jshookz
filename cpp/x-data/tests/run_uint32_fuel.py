#!/usr/bin/env python3
"""Registered UInt32 typed-view fuel and dynamic helper-route gates."""
...
MODES = (
    "uint32_retained_repeat",
    "uint32_prebound_repeat",
    "uint32_rebind_repeat",
    "uint32_raw_recertify_repeat",
)
HELPERS = (
    "u32_retained_once_c",
    "u32_prebound_once_c",
...
# Reproduced wasm32 slopes are 34 / 51 / 121 / 1448. Raising a cap is a
# review-triggering rebaseline, not a routine test repair.
CEILINGS = {"retained": 38, "prebound": 56, "rebind": 130, "raw": 1525}
...
def compile_wasm(src: Path, wasi: Path, out: Path, *, instrumented: bool) -> None:
    ...
    if instrumented:
        flags.append("-DCATL_XDATA_UINT32_HELPER_CALL_COUNTS")
    tus = [
        ...
        src / "tests" / "uint32_fuel_once.cpp",
        src / "tests" / "uint32_fuel_wasm.cpp",
    ]
    command = [
        *flags,
...
def run_mode(wasm: Path, mode: str, iterations: int) -> tuple[int, str]:
    ...
    wasi = WasiConfig()
    wasi.argv = ["uint32_fuel", mode, str(iterations)]
    with (
...
def main() -> int:
    ...
    out = args.keep_wasm or Path(tempfile.mkdtemp()) / "uint32_fuel.wasm"
    try:
        ...
        _, invalid_stdout = run_mode(out, "uint32_invalid_setup", 1)
        ...
        slopes = {
            mode: (rows[(mode, 4000)] - rows[(mode, 2000)]) / 2000
            for mode in MODES
        }
        for mode, slope in slopes.items():
            print(f"slope {mode}={slope:.3f}")
        retained = slopes["uint32_retained_repeat"]
        prebound = slopes["uint32_prebound_repeat"]
        rebind = slopes["uint32_rebind_repeat"]
        raw = slopes["uint32_raw_recertify_repeat"]
