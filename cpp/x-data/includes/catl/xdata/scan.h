...
template <bool Track> struct DupTracker;

template <> struct DupTracker<false> {
    void
...
template <> struct DupTracker<true> {
    uint64_t bits[64]{};
    uint32_t extra[16]{};
    uint8_t extra_n = 0;
    bool dup = false;
...
inline AdmitResult
admit_field(ParserContext& ctx, FieldScope scope, uint32_t field_code,
...
void scan_object(ParserContext& ctx, Protocol const& protocol, Sink& sink,
...
scan_array(ParserContext& ctx, Protocol const& protocol, Sink& sink, int depth)
{
    ...
    while (!ctx.failed() && !ctx.empty()) {
        ...
        if constexpr (Sink::kRecords) {
            sink.emit(
                FieldFrame{field_code, static_cast<uint32_t>(header_begin),
...
template <ScanMode M, class Sink>
void
scan_object(ParserContext& ctx, Protocol const& protocol, Sink& sink, int depth,
    bool top_level)
{
    if (ctx.failed())
        return;
    ...
    while (!ctx.failed() && !ctx.empty()) {
        ...
        if (vl) {
            ...
            if (ctx.failed())
                return;
        } else if (type == FieldTypes::Number.code) {
            constexpr size_t width = 12;
            if (ctx.remaining() < width) {
                ctx.fail("truncated Number");
                return;
            }
            if constexpr (M == ScanMode::CertifyWire) {
                if (!NumberRules::certify(Slice{ctx.at(), width})) {
                    ctx.fail("invalid Number representation");
                    return;
                }
            }
            if (!ctx.advance(width))
                return;
        } else {
